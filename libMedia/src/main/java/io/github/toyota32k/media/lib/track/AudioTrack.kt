...
class AudioTrack
private constructor(extractor: Extractor, inputFormat:MediaFormat, outputFormat: MediaFormat, encoder: MediaCodec, report:Report, cancellation: ICancellation)
    : Track(extractor, /*inputFormat, outputFormat,*/ Muxer.SampleType.Audio, cancellation) {
    override val decoder: AudioDecoder = AudioDecoder(inputFormat, report, cancellation).apply { start() }
    ...
    companion object {
        fun create(inPath: IInputMediaFile, strategy: IAudioStrategy, report: Report, cancellation: ICancellation): AudioTrack? {
            ...
            return AudioTrack(extractor, inputFormat, outputFormat, encoder, report, cancellation)
        }
    }
}
