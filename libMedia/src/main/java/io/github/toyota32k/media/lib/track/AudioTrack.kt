...
private constructor(extractor: Extractor, inputFormat:MediaFormat, outputFormat: MediaFormat, encoder: MediaCodec, report:Report)
    : Track(extractor, /*inputFormat, outputFormat,*/ Muxer.SampleType.Audio /*, report*/) {
    ...
    companion object {
        fun create(inPath: IInputMediaFile, strategy: IAudioStrategy, report: Report): AudioTrack? {
            val extractor = Extractor.create(inPath, Muxer.SampleType.Audio)
            if(extractor == null) {
            ...
            return AudioTrack(extractor, inputFormat, outputFormat, encoder, report)
        }
    }
}
