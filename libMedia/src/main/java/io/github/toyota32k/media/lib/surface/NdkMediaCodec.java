...
public class NdkMediaCodec implements MediaCodecWrapper {
    ...
    // patch for api<Q by toyota32k
    private int getIntegerFromFormat(MediaFormat format, String name, int defaultValue) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            return format.getInteger(name, defaultValue);
        } else {
            try {
    ...
    public void configure(MediaFormat format, int flags, Surface surface) {
        int width = getIntegerFromFormat(format, MediaFormat.KEY_WIDTH, -1);
        int height = getIntegerFromFormat(format, MediaFormat.KEY_HEIGHT, -1);
        int colorFormat = getIntegerFromFormat(format, MediaFormat.KEY_COLOR_FORMAT, -1);
        int bitRate = getIntegerFromFormat(format, MediaFormat.KEY_BIT_RATE, -1);
        int frameRate = getIntegerFromFormat(format, MediaFormat.KEY_FRAME_RATE, -1);
        int iFrameInterval = getIntegerFromFormat(format, MediaFormat.KEY_I_FRAME_INTERVAL, -1);
        int lowLatency = getIntegerFromFormat(format, MediaFormat.KEY_LOW_LATENCY, -1);
        int range = getIntegerFromFormat(format, MediaFormat.KEY_COLOR_RANGE, -1);
        int standard = getIntegerFromFormat(format, MediaFormat.KEY_COLOR_STANDARD, -1);
        int transfer = getIntegerFromFormat(format, MediaFormat.KEY_COLOR_TRANSFER, -1);
